...
###########UPDATE bash.profile##############
INST_ROOT=/data/x/tools/xcc-team
FOUND=`grep $INST_ROOT/env/xcc-team.bashrc $HOME/.bash_profile`
if test -z "$FOUND" ; then
    echo "source /data/x/tools/xcc-team/env/team.bashrc"  > /tmp/$USER.bash_profile
    cat  $HOME/.bash_profile   | sed '/xcc-team/ d' >> /tmp/$USER.bash_profile
...
if [[ -n "$NEW_VIMRC" ]]; then
    bash /data/x/tools/xcc-team/vimide/backup.sh
    rm -rf ~/.vimrc
fi
...
VIM_SRC=/data/x/tools/xcc-team/vim
ln -s $VIM_SRC/runtime  .vim

CLEAR_OLD_RC="s/.+pylon-1.4.+_vimrc//g"
TIME_RC="s/.+xcc-team.+_vimrc//g"
############UPDATE .VIMRC ################
...
/data/x/tools/xcc-team/vimide/update.sh
